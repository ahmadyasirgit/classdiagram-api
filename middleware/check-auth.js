...
module.exports = (req, res, next) => {
  ...
  let sql = `SELECT secret_key FROM users WHERE user_guid = ${db.connection.escape(
    userGuid
  )} AND is_deleted='0'`;
  let query = db.Query(sql).then(({err, result:results}) => {
    ...
    if (JWT_KEY) {
      try {
        ...
        let userData = `SELECT * FROM users WHERE user_guid = ${db.connection.escape(
          userGuid
        )} AND is_deleted='0'`;
        let query1 = db.Query(userData).then(({err, result:response}) => {
